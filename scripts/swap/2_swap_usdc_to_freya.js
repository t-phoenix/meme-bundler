...
async function main(){
    ...
    // INPUT: AMOUNT
    const inputAmount = 1_000000; // 1 USDC
    ...
    console.log("Quote Output for", Number(inputAmount)/10**6, " is", outputAmount/10**18)
    // const sqrtPriceCalculated = priceToSqrtPriceX96(outputAmount/10**6)
    // console.log("Price to Sqrt Price:", sqrtPriceCalculated)
    
    // Approve Tokens
    // CHECK APPROVAL
    await USDC_Contract.connect(swappingWallet).approve(uniswapAddresses.swapRouter2Address, Number(inputAmount))
