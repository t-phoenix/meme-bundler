...
async function main() {
  ...
  for (let index = 0; index < inputAmountList.length; index++) {
    // INPUT: AMOUNT
    const inputAmount = hre.ethers.parseUnits(inputAmountList[index], 6); // FREYA
    ...
    const swapRoute = new Route([FREYAUSDCPool], USDC_TOKEN, FREYA_TOKEN);

    // Getting a Quote for Swap
    // Using SwapQuoter from v3-sdk
    const { calldata } = await SwapQuoter.quoteCallParameters(
      swapRoute,
      CurrencyAmount.fromRawAmount(USDC_TOKEN, Number(inputAmount)),
