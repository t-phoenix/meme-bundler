...
async function main(){
    ...
    // INPUT: SWAP TOKEN
    // const USDC_Contract = new hre.ethers.Contract(tokenAddresses.WETH_Address, ERC20_ABI, hre.ethers.provider)
    ...
    console.log("Quote Output for", Number(inputAmount)/10**18, " is", Number(decodedData[0])/10**18)

    // -----------------------------------//
    // APPROVE TOKENS
    // CHECK APPROVAL
    // -----------------------------------//
    const approveFreya = await FREYA_Contract.connect(swappingWallet).approve(uniswapAddresses.swapRouter2Address, inputAmount)
