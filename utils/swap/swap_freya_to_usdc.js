...
const { USDC_TOKEN, uniswapAddresses, tokenAddresses, MAX_FEE_PER_GAS, MAX_PRIORITY_FEE_PER_GAS, WBTC_TOKEN, FREYA_TOKEN } = require('../constants');
const IUniswapV3PoolABI = require('@uniswap/v3-core/artifacts/contracts/interfaces/IUniswapV3Pool.sol/IUniswapV3Pool.json');
const { ERC20_ABI } = require('../ABI');
const { getAccounts, printBalances } = require('../helper');
const UniswapV3Router2ABI = require('../UniswapV3Router2ABI.json');
const { quote_freya_to_usdc } = require('../quote/quote_freya_to_usdc');


async function swap_freya_to_usdc(_wallet, _inputAmt){
    ...
    const quoteSwap = await quote_freya_to_usdc(inputAmount);
    console.log("Outer Quote Output for", Number(inputAmount)/10**18, " TOKEN is", Number(quoteSwap)/10**6, " USDC")
